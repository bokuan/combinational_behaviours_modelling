...
sys.path.append('/mnt/vasp-got-da/scripts/common_libraries/dsl_helpers')
from cnx_dsl3_database import EngineCreator  # noqa: E402

service_login = False
try:
    with open('/mnt/vasp-got-da/scripts/team_data_analytics/therese/config/.dsl3_db_config.json') as f:
        db_info = json.load(f)
        service_login = True
except FileNotFoundError:
    pass

user_vcn = getpass.getuser()
...
def write_df_to_db(df: pd.DataFrame, table_name: str, schema: str = 'da_dev',
    ...
    _log_action('write_df_to_db', ['table_name: ' + table_name,
                                   'schema: ' + schema,
                                   'append: ' + str(append),
...
def replace_rows(table_name: str, schema: str,
    ...
    **P.P.S This script looks at each row you are "replacing" individually,
    ...
    # If there is more than one set of conditions to remove rows based on
        ...
        _log_action('replace_rows', ['table_name: ' + table_name,
                                     'schema: ' + schema,
...
def replace_values_on_key(table_name: str, schema: str,
    ...
    _log_action('replace_values_on_key', ['table_name: ' + table_name,
                                          'schema: ' + schema,
                                          'update_dict: ' + str(update_dict),
                                          'identifying_columns: ' +
...
def _log_action(func_called: str, passed_args: List[str]):
    ...
    splitter = '\n' + '-'*50 + '\n'
    lines_to_log = [current_time, user_vcn,
...
def __create_engine() -> Engine:
    ...
    try:
        ...
        engine = engine_creator.get_engine(database)
        print("Connection successful")
        return engine

    except Exception as e:
        print(f"An error occurred: {e}")
        print('Too many unsuccessful attempts. Authentication failed.')
        return None
